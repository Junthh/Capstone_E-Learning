...
const schema = z.object({
  ...
  hinhAnh: z
    .string()
    .min(1, "Vui lòng dán URL ảnh")
    .regex(
      imageUrlPattern,
      "Hình ảnh phải là URL http/https hợp lệ (.png/.jpg/.jpeg/.webp/.gif)"
    ),
...
<div className={styles.wrap}>
  ...
  >
    {/* Hàng 1: 2 ô */}
    <InputWithIcon id="maKhoaHoc" label="Mã khóa học" icon={<Tag size={14} />} placeholder="VD: NODEJS_01"
      {...register("maKhoaHoc")} error={errors.maKhoaHoc?.message} readOnly />

    <InputWithIcon id="tenKhoaHoc" label="Tên khóa học" icon={<FileText size={14} />} placeholder="VD: NodeJS Căn bản"
      {...register("tenKhoaHoc")} error={errors.tenKhoaHoc?.message} />

    {/* Hàng 2: Danh mục (1/2) + Ngày tạo (1/2) */}
    <div className={`${styles.divSelect}`}>
      <Label htmlFor="maDanhMucKhoaHoc" className={`text-lg ${styles.label}`}>Danh mục khóa học</Label>
      <div className={`flex items-center rounded-md border border-gray-300 bg-white overflow-hidden ${styles.selectGroup}`}>
        <span className={`bg-gray-50 text-gray-500 border-r border-gray-200 flex items-center ${styles.leading}`}>
          <List size={14} />
        </span>
        <Controller
          control={control}
          ...
          render={({ field }) => (
            <Select onValueChange={field.onChange} value={field.value} disabled={loadingCats || catErr}>
              ...
              <SelectContent>
                {categories.map((c) => (
                  <SelectItem key={c.maDanhMuc} value={c.maDanhMuc} className={`text-sm ${styles.selectItem}`}>
                    {c.tenDanhMuc}
                  </SelectItem>
                ))}
              </SelectContent>
    ...
    <InputWithIcon id="ngayTao" label="Ngày tạo" type="date" icon={<Calendar size={14} />} placeholder=""
      {...register("ngayTao")} error={errors.ngayTao?.message} />

    {/* Hàng 3: Bí danh + Lượt xem */}
    <InputWithIcon id="biDanh" label="Bí danh" icon={<BookOpen size={14} />} placeholder="VD: nodejs-canban"
      {...register("biDanh")} error={errors.biDanh?.message} />

    <InputWithIcon id="luotXem" label="Lượt xem" type="number" icon={<Eye size={14} />} placeholder="0"
      {...register("luotXem")} error={errors.luotXem?.message} />

    {/* Hàng 4: Đánh giá + (để trống/hoặc thêm ô khác nếu có) */}
    <InputWithIcon id="danhGia" label="Đánh giá" type="number" icon={<Star size={14} />} placeholder="0"
      {...register("danhGia")} error={errors.danhGia?.message} />

    {/* spacer nếu muốn cân lưới */}
    <div></div>

    {/* Full width: Tài khoản người tạo */}
    <div className={`md:col-span-2 ${styles.span2}`}>
      <Label className={`text-lg ${styles.label}`}>Tài khoản người tạo</Label>
      <div className={`${styles.userInput} mt-1 w-full rounded-md border border-gray-300 px-3 py-2 bg-gray-50 text-sm`}>
    ...
    {/* Full width: Hình ảnh */}
    <div className={`md:col-span-2 ${styles.span2}`}>
      <Label htmlFor="hinhAnh" className={`text-lg ${styles.label}`}>Hình ảnh</Label>
      <div className="flex gap-3 items-start mt-1">
        <div className="flex-1">
          <InputWithIcon id="hinhAnh" label="" icon={<ImageIcon size={14} />}
      ...
      {imageUrlPattern.test(watch("hinhAnh") || "") && (
        <img src={watch("hinhAnh")} alt="preview"
    ...
    <div className={`md:col-span-2 flex flex-wrap items-center justify-end gap-2 text-lg ${styles.span2}`}>
      ...
      <Button type="button" className={`text-lg h-11 bg-red-600 text-white hover:bg-red-700 cursor-pointer ${styles.btn}`}
